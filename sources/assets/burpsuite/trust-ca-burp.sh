...
function trust_ca_burp_in_firefox() {
  ...
  if [[ -d "/opt/tools/BurpSuiteCommunity/" ]]; then
    ...
    echo y|/root/.asdf/installs/java/openjdk-21/bin/java -Djava.awt.headless=true -jar /opt/tools/BurpSuiteCommunity/BurpSuiteCommunity.jar --config-file=/opt/tools/BurpSuiteCommunity/conf.json 2>&1 > /dev/null &
